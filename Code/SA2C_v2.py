...
# Add my arguments to use this as a function instead of from the command line
def parse_args(myArgs):
    ...
    """ Adding a lambda argument as a mixing factor for Item feature inclusion and cold-start
    per research paper Temporal-Contextual Recommendation in Real-Time by Yifei Ma et al. 
    (https://assets.amazon.science/96/71/d1f25754497681133c7aa2b7eb05/temporal-contextual-recommendation-in-real-time.pdf)"""
    parser.add_argument(
        "--lambda_factor",
        default=0.5,
        type=float,
        help="lambda mixing factor for Item feature inclusion and cold-start",
    )
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="number of item features (for cold-start)",
    )
...
class QNetwork:
    def __init__(
        ...
        state_size,
        pretrain,
        args=None,  # Added to pass in arguments
        name="DQNetwork",
    ):
        ...
        self.clip = args.clip
        self.item_feature_size = args.item_feature_size
        # self.save_file = save_file
        self.model = args.model
        self.is_training = tf.compat.v1.placeholder(tf.bool, shape=())
        self.name = name
        self.lr_2 = args.lr_2
        # Adding lambda factor as a mixing factor for Item feature inclusion and cold-start to the model
        self.lambda_factor = args.lambda_factor
        # Asserting lambda is between 0 and 1
        assert (
            self.lambda_factor >= 0 and self.lambda_factor <= 1
        ), "Lambda factor must be between 0 and 1 (inclusive)"
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.feature_emb = tf.matmul(
                tf.cast(self.feature_inputs, tf.float32),
                self.all_embeddings["item_embeddings"],
            )
            ...
            self.output1_original = tf.compat.v1.layers.dense(
                self.states_hidden, self.item_num, activation=None
            )  # all q-values

            self.output2_original = tf.compat.v1.layers.dense(
                self.states_hidden, self.item_num, activation=None
            )  # all ce logits

            # Using Item features per Temporal-Contextual Recommendation in Real-Time by Yifei Ma et al.
            self.output1 = (
...
def evaluate(sess, QN_1, args):  # Updated arguments to use outside of main
    ...
    while evaluated < len(eval_ids):
        ...
        prediction = sess.run(
            ...
            feed_dict={
                ...
                QN_1.feature_inputs: item_properties,
            },
        )
...
def train(myArgs):  # Updated arguments to use outside of main
    # Network parameters
    ...
    QN_1 = QNetwork(
        ...
        pretrain=False,
        args=args,  # Added to pass in arguments
    )
    QN_2 = QNetwork(
        name="QN_2",
        hidden_size=args.hidden_factor,
        ...
        for j in range(num_batches):
            ...
            target_Qs = sess.run(
                ...
                feed_dict={
                    ...
                    target_QN.feature_inputs: item_properties,
                },
            )
            target_Qs_selector = sess.run(
                ...
                feed_dict={
                    ...
                    mainQN.feature_inputs: item_properties,
                },
            )
